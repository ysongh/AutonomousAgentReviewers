...
- `bootstrap-filecoin/` — Phase 0 spike for the proposed **Demo Judge**
  ...
  — NOT wired into the agents/ pipeline.** Phase 2 productionized this spike
  into `shared/filecoin-storage.js` (see Demo Judge below). See
  `bootstrap-filecoin/README.md`.
...
- `bootstrap-demojudge/` — Phase 1 spike for the proposed **Demo Judge**
  ...
  pipeline.** Phase 2 productionized this spike into `agents/judge-demo`
  (see Demo Judge below). See `bootstrap-demojudge/README.md`. Pipeline:
...
- `shared/` — Common modules used by every agent: `og-storage.js` (the
  productionized 0G workaround — see below), `filecoin-storage.js` (the
  productionized Filecoin Warm Storage video client — Phase 2; see Demo
  Judge below), `agent-wallet.js` (per-agent signer factory — see Per-agent
  wallets below), `schemas.js` (zod), `claude.js` (Anthropic wrapper),
  `github.js` (Octokit wrapper), `logger.js` (pino → `logs/<agent>.jsonl`),
  `config.js` (ports, agent IDs). Has its own `package.json`. Agents depend on it via
...
- `agents/<name>/` — One Express process per agent, own `package.json`, own
  ...
  (4004), `aggregator` (4005). Phase 2 adds `judge-demo` (4006) — the demo
  video reviewer (see Demo Judge below). Ports + URLs are centralized in
  `shared/config.js` (`PORTS`, `AGENT_IDS`, `JUDGE_URLS`,
  `JUDGE_REVISE_URLS`, `AGGREGATOR_URL`, `JUDGE_DEMO_URL`).
...
- Root `.env.agents` — gitignored, generated by
  `scripts/generate-agent-wallets.js`. Contains six per-agent keypairs
  (`INTAKE_PRIVATE_KEY` / `INTAKE_ADDRESS`, `JUDGE_TECHNICAL_*`,
  `JUDGE_ORIGINALITY_*`, `JUDGE_SKEPTIC_*`, `AGGREGATOR_*`,
  `JUDGE_DEMO_*` — the last added in Phase 2).
...
https://faucet.0g.ai. `.env.agents` (gitignored) holds all six
`<PREFIX>_PRIVATE_KEY` / `<PREFIX>_ADDRESS` pairs (intake, the three
judges, the aggregator, and — added in Phase 2 — `judge-demo`).
`start-all.sh` sources both `.env` and `.env.agents` so every spawned
agent process inherits its own key.
...
- `node scripts/check-agent-balances.js` — prints each address's balance
  and exits 0 only if all six are ≥ 0.04 0G (gas buffer above the 0.05
  faucet drip).
...
- `SubmissionRecord` — what intake uploads and what each judge downloads.
  Phase 2 added two **nullable** fields: `demoVideoPieceCid` and
  `demoVideoRetrievalUrl`. Both are `null` for submissions without a video
  (the pipeline then behaves byte-for-byte as before); when a video is
  supplied, intake uploads it to Filecoin and sets both. Existing readers
  parse fine — the demo judge is the only consumer that requires them
  non-null.
...
- **ESM-only SDK from a CommonJS module.** `@filoz/synapse-sdk` (v0.41+)
  and its required peer dep `viem` are **ESM-only**; `shared/` and the
  agents are CommonJS. The module bridges the boundary with **dynamic
  `import()`** (cached), never `require()`. Do not convert the agents to
  ESM to "fix" this.
- **ethers→viem.** No `Synapse.create({ privateKey })`; build a local
  `privateKeyToAccount(0x…)` and pass it as `account`. The `calibration`
  chain is baked into the SDK.
...
### agents/judge-demo — the reviewer

Standard three-file agent (`index.js`, `handler.js`, `prompt.js`) on port
**4006** with its own 0G wallet (`JUDGE_DEMO_*` in `.env.agents`). It does
**NOT** import `@filoz/synapse-sdk` — it fetches the video over plain HTTP
from `retrievalUrl`. `POST /review { submissionRootHash, submissionId }`:
1. `downloadJSON(submissionRootHash)` → zod `SubmissionRecord`; **400** if
   `demoVideoRetrievalUrl`/`demoVideoPieceCid` are null (caller bug — intake
   must not route a video-less submission here).
2. HTTP-**stream** the mp4 to a temp file (never buffered; **150MB cap**,
   enforced up front via `content-length` and mid-stream by byte count).
3. Stage A — `ffprobe` duration → `min(14, ceil(dur/15))` frames @768px JPEG
   (hard cap 14), recording each `MM:SS`. Stage B — `ffmpeg` 16kHz mono →
   `whisper-1` `verbose_json` segment-timestamped transcript. Stage C — ONE
   forced-tool_use Claude call (`submit_demo_verdict`) via the shared
   `callJudge` (multimodal `content` array + `maxTokens: 1500`), with **real
   REPO CONTEXT** (the SubmissionRecord's capped README + repo metadata) so
   `claims_check` tests README *and* narration claims against the frames.
4. Build `DemoVerdict` (adds `agentId`/`submissionId`/`videoPieceCid`/
   `producedAt`), zod-validate, `uploadJSON` on the judge-demo wallet →
   `{ demoVerdictRootHash }`.
5. **Temp files (video + frames + audio) are cleaned in a `finally` block.**

Same discipline as the spike: ONE Claude call, no retries, no multi-pass;
`ffmpeg-static` allowlisted in `pnpm.onlyBuiltDependencies`; `openai` pinned
...
node scripts/check-agent-balances.js   # exits 0 once all six are >= 0.04 0G
```
...
adding a new agent (like the aggregator, or Phase 2's judge-demo) doesn't
invalidate the already-funded wallets. `--force` regenerates everything and
DOES invalidate them; don't pass it unless you intend to refund all six.
...
- `filecoin-upload-start`, `filecoin-upload-complete` — Phase 2, intake's
  Filecoin Warm Storage video upload (carries `uploadMs`/`sizeBytes`)
- `demo-review-start`, `demo-review-complete` — Phase 2, the intake →
  judge-demo `/review` hop lifecycle
- `demo-video-download-start`, `demo-video-download-complete` — Phase 2,
  judge-demo fetching the mp4 over plain HTTP from the retrievalUrl
- `frames-extract-complete` — Phase 2, judge-demo Stage A keyframe extraction
