...
- Root `.env.agents` — gitignored, generated by
  `scripts/generate-agent-wallets.js`. Contains four per-agent keypairs
  (`INTAKE_PRIVATE_KEY` / `INTAKE_ADDRESS`, `JUDGE_TECHNICAL_*`,
  `JUDGE_ORIGINALITY_*`, `JUDGE_SKEPTIC_*`). `start-all.sh` sources both
  `.env` and `.env.agents` so every agent process inherits its own key.
  The legacy `PRIVATE_KEY` in `.env` stays put and is used only by
  `bootstrap/` scripts and `shared/smoke.js` via
  `og-storage.getDefaultSigner()`. Agents do **not** read `PRIVATE_KEY`
  any more.
...
https://faucet.0g.ai. `.env.agents` (gitignored) holds all four
`<PREFIX>_PRIVATE_KEY` / `<PREFIX>_ADDRESS` pairs. `start-all.sh` sources
both `.env` and `.env.agents` so every spawned agent process inherits its
own key.
...
Operational scripts:
- `node scripts/generate-agent-wallets.js [--force]` — creates
  `.env.agents` and prints the four addresses. Refuses to overwrite
  without `--force`.
- `node scripts/check-agent-balances.js` — prints each address's balance
  and exits 0 only if all four are ≥ 0.04 0G (gas buffer above the 0.05
  faucet drip).
...
node scripts/check-agent-balances.js   # exits 0 once all four are >= 0.04 0G
```
`.env.agents` is gitignored and only ever lives on disk — re-running the
generator with `--force` invalidates the existing wallets, so don't do
that unless you intend to refund.
